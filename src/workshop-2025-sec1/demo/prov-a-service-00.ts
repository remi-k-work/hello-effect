...
const InMemoryCache = Cache.of({
  lookup: (key) => Effect.fromNullable(store.get(key)).pipe(Effect.catchTag("NoSuchElementException", () => new CacheMissError({ key }))),
});
...
const runnable = program.pipe(Effect.provideService(Cache, InMemoryCache));
