...
const FileSystemCache = Cache.of({
  lookup: (key) =>
    Effect.tryPromise({
      try: () => fs.readFile(`src/workshop-2025-sec1/demo/${key}`, "utf-8"),
...
const program = Effect.gen(function* () {
  ...
  yield* subProgramOne.pipe(Effect.provideService(Cache, InMemoryCache));
  // Execute the second sub-program with the file system cache
  yield* subProgramTwo.pipe(Effect.provideService(Cache, FileSystemCache));
});
