...
export const makeImmunityTokenManager = Effect.gen(function* () {
  ...
  yield* Effect.sync(() => tokens.clear()).pipe(Effect.schedule(Schedule.cron("0 0 * * *")), Effect.forkScoped, Effect.interruptible);

  const getBalance = Effect.fn("ImmunityTokenManager.getBalance")(function* (childName: string) {
    return yield* Effect.fromNullable(tokens.get(childName)).pipe(Effect.orElseSucceed(() => 0));
  });

  const awardToken = Effect.fn("ImmunityTokenManager.awardToken")(function* (childName: string, options: { readonly reason: string }) {
  ...
  const useToken = Effect.fn("ImmunityTokenManager.useToken")(function* (childName: string) {
...
export const ImmunityTokenManagerLayer = Layer.scoped(ImmunityTokenManager, makeImmunityTokenManager);
