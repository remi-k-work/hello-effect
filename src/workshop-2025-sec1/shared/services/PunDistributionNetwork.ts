...
export const makePunDistributionNetwork = Effect.gen(function* () {
  ...
  const getChannel = Effect.fn("PunDistributionNetwork.getChannel")(function* (misbehavior: Misbehavior) {
    yield* Effect.log(`Detected new misbehavior for child: ${misbehavior.childName}`).pipe(Effect.annotateLogs({ ...misbehavior }));
  ...
  const deliverPun = Effect.fn("PunDistributionNetwork.deliverPun")(function* (pun: Pun, misbehavior: Misbehavior, channel: Channel) {
...
export const PunDistributionNetworkLayer = Layer.effect(PunDistributionNetwork, makePunDistributionNetwork).pipe(Layer.provide(PunsterClientLayer));
