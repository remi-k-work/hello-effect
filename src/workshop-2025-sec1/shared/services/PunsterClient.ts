...
export class PunsterClient extends Context.Tag("Punsterclient")<
  PunsterClient,
  {
    readonly createPun: (misbehavior: Misbehavior) => Effect.Effect<Pun, ChildImmuneError | MalformedPunError | PunsterFetchError>;
    readonly evaluatePun: (pun: Pun, misbehavior: Misbehavior, channel: Channel) => Effect.Effect<string, PunsterFetchError>;
  }
>() {}
...
export const makePunsterClient = Effect.gen(function* () {
  ...
  const httpClientOk = httpClient.pipe(
    HttpClient.mapRequest(flow(HttpClientRequest.prependUrl(apiUrl.toString()), HttpClientRequest.bearerToken(apiToken), HttpClientRequest.acceptJson)),
...
export const PunsterClientLayer = Layer.effect(PunsterClient, makePunsterClient).pipe(Layer.provide(ImmunityTokenManagerLayer));
...
function formatMetricValue(value: number, maxValue: number = 100): AnsiDoc.AnsiDoc {
...
function formatProgressBar(value: number, maxValue: number = 100, length: number = 20): AnsiDoc.AnsiDoc {
  ...
  return Doc.text(bar).pipe(Doc.cat(Doc.text(empty)), Doc.squareBracketed, Doc.annotate(color), Doc.catWithSpace(Doc.text(percentage.toFixed(1))));
}
...
function createReport(report: PunDeliveryReport): string {
  ...
  const divider = Doc.text("-".repeat(80)).pipe(Doc.annotate(Ansi.blackBright), Doc.surround(Doc.hardLine, Doc.hardLine));
  ...
  const executiveSummary = Doc.vcat([formatSubheader("EXECUTIVE SUMMARY"), Doc.empty, ...makeLines(report.executiveSummary, 80).map((line) => Doc.text(line))]);
  ...
  const observations = Doc.vcat([
    ...
    ...report.recommendations.flatMap((observation, index) => {
      ...
      return [
        Doc.text(`${index + 1}.`).pipe(Doc.annotate(Ansi.magenta), Doc.cat(Doc.space), Doc.cat(Doc.text(lines[0]!))),
